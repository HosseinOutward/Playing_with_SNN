...
class IF(LIF):
    def change_u(self, I_t):
        self.U += self.R * I_t * self.dt / self.tau
    ...
    def simulate_one_step(self, I_t):
        ...
        if self.last_fired or self.ref_time!=0:
            self.last_fired=False
            self.U=self.U_reset+(self.U-self.U_reset)%(-self.U_reset+self.theta)
            return self.U, I_t
    ...
    def dirac(self):
        a=int(self.last_fired) * (self.U-self.U_reset)//(-self.U_reset+self.theta)
        # if a!=0:print(a)
        return a
