...
if __name__ == "__main__":
    ...
    class CustomModel(FullyConnectedPopulation):
        def decide_weight(self):
            a=random.uniform(1, 40)/40
            print(a)
            return a

        def create_network(self):
            self.pre_neurons = self.neurons[:-2]
            self.post_neurons = self.neurons[-2:-1]
            self.aa = self.neurons[-1:]
            self.a = 0
            ...
            self.connect_neurons(self.post_neurons[0], self.aa[0], 1)

        def simulate_network_one_step(self, pre_I_t, post_I_t):
            u_history = []
            i_history = []
            for i,neuron in enumerate(self.pre_neurons):
                inter_U, curr = neuron.simulate_one_step(pre_I_t+i*6000)
            ...
            for neuron in self.post_neurons:
                ...
                u_history.append(inter_U)
                i_history.append(curr)
            for neuron in self.aa:
                inter_U, curr = neuron.simulate_one_step(0)
                if neuron.last_fired: self.a += 1
                u_history.append(inter_U)
                i_history.append(curr)
            ...
            if neuron.internal_clock % 1000 == 0: print(neuron.internal_clock)
